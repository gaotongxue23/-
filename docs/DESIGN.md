...
本文件提炼自 [OpenDesign · Claude pack](https://opendesign.cc/packs/claude/) 的 11 层设计规格，作为八字命理学的设计语言基线。**只迁移设计语言（颜色/字体/间距/形状/动效/语气等 tokens 与原则），不复制任何品牌资产、Logo、插画或文案。** 文中所有示例文案均为八字命理学自有场景。
...
- **落到八字命理学**：命理解读是"复杂思想徐徐展开"的场景，界面应像一张安静、有质感的纸，托住内容而不抢戏。
...
- **CTA**：直接简短、动词开头（八字命理学场景示例：「开始排盘」「查看命盘」「请大师解读」）。
...
下方为这套设计语言的完整变量基线。八字命理学现有 `src/styles.css` 已对齐**颜色 / 圆角 / 阴影 / 缓动 / 字体 / 字阶**；以下用 `/* 待补 */` 标出当前缺失、建议补入的变量（间距阶梯、动效时长、布局容器），以统一硬编码数值。
