...
- 优化专业报告与流派会诊导出体验，PDF 现在支持直接下载，不再默认跳转打印页手动另存。
- 修复图片导出时浏览器提示 `Tainted canvases may not be exported` 的问题，改为纯 Canvas 长图绘制。
- 新增报告导出美化模板，支持 PDF 与 PNG 长图导出，并自动隐藏用户不应看到的内部字段依据。
- 命盘页点击“命盘速览”会直接跳转到解读页生成内容，且命盘速览不作为正式报告导出。
- 报告封面与四柱区域视觉优化：徽标对齐修正，四柱按五行使用彩色天干/地支卡片展示。
- 本版本仅调整导出、展示与报告模板层，没有修改大师断事逻辑或角色提示词。
...
- 报告导出：专业报告与流派会诊可导出为 PDF 或 PNG 长图。
